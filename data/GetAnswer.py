...
def getAnswer(tokenizer_result, condition_x_input, condition_x_result, coarse_fine_input, coarse_fine_result, output_file):
    with open(tokenizer_result, 'r', encoding='utf-8') as f_r1, open(condition_x_input, 'r', encoding='utf-8') as f_r2, \
        ...
        cnt = 0
        while True:
            js = {"context":"", "question":"", "condition":[], "coarse":[], "fine":[], "condition_coarse":[], "condition_fine":[], "coarse_fine":[]}
            raw_data = f_r1.readline()
            if not raw_data:
                break
            cnt += 1 
            raw_data = json.loads(raw_data)
            js["context"], js["question"], js["condition"], js["coarse"], js["fine"] = raw_data["context"], raw_data["question"], raw_data["condition"], raw_data["coarse"], raw_data["fine"]
