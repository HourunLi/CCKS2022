...
def prepareTrainingData(file_name, output_file_condition_x, output_file_coarse_fine):
    with open(file_name, 'r', encoding='utf-8') as f_r, open(output_file_condition_x, "w", encoding="utf-8") as f_w1, \
        ...
        while True:
            ...
            condition_coarses, condition_fines, coarse_fines = js[
                "condition_coarse"], js["condition_fine"], js["coarse_fine"]
            ...
            for (relation1s, relation2s, type) in [(conditions, coarses, "condition_coarse"), (conditions, fines, "condition_fine"), (coarses, fines, "coarse_fine")]:
                # match all the potential relations between arbitrary two answers.
                matchPotentialRelation(
                    relation1s, relation2s, context, f_w1, f_w2, type, relation_set)
...
def matchPotentialRelation(tag1s, tag2s, context, f_w1, f_w2, type, relation_set, index=0):
    ...
    for tag1 in tag1s:
        for tag2 in tag2s:
            pos_tag = [[tag1[1][0], '[unused1]'], [tag1[1][1], '[unused2]'], [
                tag2[1][0], '[unused3]'], [tag2[1][1], '[unused4]']]
            pos_tag.sort()
            begin, end = pos_tag[0][0], pos_tag[3][0]
            # Question is ahead of context.
            # We need to find the begin location of context
            context_begin, context_end = 0, len(context)
            for i in range(begin, 0, -1):
                if context[i] == '\n':
                    context_begin = i + 1
            ...
            for i in range(end, len(context), 1):
                if context[i] == '\n':
                    context_end = i
                    break
            # Add special symbols to mark the answer's location.
            # More specific info can be found in paper and report.pptx
            js["sentence1"] = context[context_begin:pos_tag[0][0]] + pos_tag[0][1] + context[pos_tag[0][0]:pos_tag[1][0]] + pos_tag[1][1] + \
                context[pos_tag[1][0]:pos_tag[2][0]] + pos_tag[2][1] + context[pos_tag[2]
                                                                               [0]:pos_tag[3][0]] + pos_tag[3][1] + context[pos_tag[3][0]:context_end]
...
# process the predict data generated by sequence labeling module.
# It forms all the potential relations.
def predictDataPreprocess(token_predict_result, output_file_condition_x, output_file_coarse_fine):
    ...
    with open(token_predict_result, 'r', encoding='utf-8') as f_r, \
        ...
        while True:
            ...
            context, conditions, coarses, fines = prediction["context"], prediction[
                "condition"], prediction["coarse"], prediction["fine"]
            matchPotentialRelation(
                conditions, coarses, context, f_w1, f_w2, relationType[0], [], cnt)
            matchPotentialRelation(
                conditions, fines, context, f_w1, f_w2, relationType[1], [], cnt)
            matchPotentialRelation(
                coarses, fines, context, f_w1, f_w2, relationType[2], [], cnt)
    f_r.close()
    f_w1.close()
    f_w2.close()
